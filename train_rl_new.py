...
import train_bc
from lav.progress import ProgressEstimator, set_demo_embs,RewardTransformer,set_group_demo_embs,tensor_to_images
...
class Workspace:
    def __init__(self, cfg: MainConfig, from_main=True):
        ...
        self.estimator = ProgressEstimator(cfg.est_ckpt_path,"cuda:0",cfg.est_k)
        set_group_demo_embs(cfg.img_demo_path,self.estimator,"cuda:0")
        self.reward_transformer = RewardTransformer(self.estimator,cfg.episode_length,gamma=cfg.discount)
    ...
    def warm_up(self):
        ...
        while True:
            ...
            self.reward_transformer.push(reward,image_obs["agentview"])
            reply = {"action": action}
            # self.replay.add(obs, reply, reward, terminal, success, image_obs)
            if not terminal:
                self.replay.add(obs, reply, reward, terminal, success)
            else:
                transed_rewards = self.reward_transformer.flush()
                self.replay.add(obs, reply, reward, terminal, success,trans_rewards=transed_rewards)
            if terminal:
                num_episode += 1
    ...
    def rl_train(self, stat: common_utils.MultiCounter):
        ...
        for i in range(self.cfg.num_critic_update):
            ...
            # tensor_to_images(batch.obs["agentview"][:5,...]/255.0,save_dir="/media/mani/Data/gyy_workspace/IBRL/ibrl/test/check")
