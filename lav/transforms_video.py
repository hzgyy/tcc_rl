...
class ResizeVideo(object):
    ...
    Args:
        size (sequence or int): Desired output size. If size is a sequence like
            ...
            (size * height / width, size)✅ 四、视频版的 NormalizeVideo 和 图像版 Normalize 的区别
tv.Normalize() → 用于单帧图像（C×H×W）

tv.NormalizeVideo() → 用于视频序列（T×C×H×W）

两者的逻辑相同，只是多了时间维度（T 维度沿用同样的归一化参数）。
